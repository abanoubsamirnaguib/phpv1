...
class Database 
{
    ...
    private  $username = "planingsolution";
    private  $password = "111222333";
    private  $db = "planingsolution";
    ...
    // update data in db 
    public function delete($table,$id, $Path)
    {
        $sql = "DELETE FROM $table WHERE `id`='$id' ";
        $result = mysqli_query($this->conn,$sql);
        if(mysqli_query($this->conn,$sql))
        { 
            if (file_exists("img/" . $Path) && $Path != "default.jpg") {             
                    unlink("img/" . $Path);        
            }
